...
![1_自媒体端后台搭建](image/Chapter_3/1_自媒体端后台搭建.png)
...
![2_自媒体端素材上传需求分析](image/Chapter_3/2_自媒体端素材上传需求分析.png)
...
![3_自媒体端素材上传实现思路](image/Chapter_3/3_自媒体端素材上传实现思路.png)
...
在leadnews-wemedia中新增拦截器

```java
package com.leadnews.wemedia.interceptor;

import com.leadnews.model.wemedia.pojos.WmUser;
import com.leadnews.utils.thread.WmThreadLocalUtils;
...
### 1.2.4 自媒体微服务集成leadnews-file-starter

①：导入leadnews-file-starter
...
在自媒体端WemediaApplication启动类中添加mybatis-plus的分页拦截器
...
![4_自媒体端查询所有频道](image/Chapter_3/4_自媒体端查询所有频道.png)
...
![5_自媒体端查询所有文章需求分析](image/Chapter_3/5_自媒体端查询所有文章需求分析.png)
...
![6_自媒体端文章发布需求分析](image/Chapter_3/6_自媒体端文章发布需求分析.png)



### 1.6.2 实现思路

![7_自媒体端文章发布实现思路](image/Chapter_3/7_自媒体端文章发布实现思路.png)
...
```json
{
    ...
    {
        "type":"text",
        "value":"随着智能手机的普及，人们更加习惯于通过手机来看新闻。由于生活节奏的加快，很多人只能利用碎片时间来获取信息，因此，对于移动资讯客户端的需求也越来越高。新闻资讯项目正是在这样背景下开发出来。新闻资讯项目采用当下火热的微服务+大数据技术架构实现。本项目主要着手于获取最新最热新闻资讯，通过大数据分析用户喜好精确推送咨询新闻"
    },
    {
        "type":"image",
...
<mapper namespace="com.leadnews.wemedia.mapper.WmNewsMaterialMapper">
