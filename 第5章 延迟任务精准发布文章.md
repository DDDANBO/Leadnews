...
![1_延迟队列](image/Chapter_5/1_延迟队列.png)
...
![2_DelayQueue](image/Chapter_5/2_DelayQueue.png)
...
#### 1.1.2.2 RabbitMQ实现延迟任务

- TTL：Time To Live (消息存活时间)

- 死信队列：Dead Letter Exchange(死信交换机)，当消息成为Dead message后，可以重新发送另一个交换机（死信交换机）

![3_RabbitMQ实现延迟任务](image/Chapter_5/3_RabbitMQ实现延迟任务.png)
...
![4_redis实现延迟任务](image/Chapter_5/4_redis实现延迟任务.png)
...
![5_redis实现延迟任务实现思路](image/Chapter_5/5_redis实现延迟任务实现思路.png)
...
③ 添加CacheService到leadnews-common模块下，并添加自动配置src/main/resources/META-INF/spring.factories
...
org.springframework.boot.autoconfigure.EnableAutoConfiguration=\
  ...
  com.leadnews.common.redis.CacheService,\
  com.leadnews.common.jackson.InitJacksonConfig
```
...
#### 1.3.6.1 redis key值匹配

方案1：keys 模糊匹配

keys的模糊匹配功能很方便也很强大，但是在生产环境需要慎用！开发中使用keys的模糊匹配却发现redis的CPU使用率极高，所以公司的redis生产环境将keys命令禁用了！redis是单线程，会被堵塞。

![6_redis_keys模糊匹配](image/Chapter_5/6_redis_keys模糊匹配.png)

方案2：scan 

SCAN 命令是一个基于游标的迭代器，SCAN命令每次被调用之后， 都会向用户返回一个新的游标， 用户在下次迭代时需要使用这个新游标作为SCAN命令的游标参数， 以此来延续之前的迭代过程。

![7_redis_scan命令](image/Chapter_5/7_redis_scan命令.png)
...
![8_普通redis客户端和服务器交互模式](image/Chapter_5/8_普通redis客户端和服务器交互模式.png)

Pipeline请求模型

![9_Pipeline请求模型](image/Chapter_5/9_Pipeline请求模型.png)

官方测试结果数据对比

![10_官方测试结果数据对比](image/Chapter_5/10_官方测试结果数据对比.png)
...
### 1.3.7 分布式锁解决集群下的方法抢占执行

#### 1.3.7.1 问题描述

启动两台leadnews-schedule服务，每台服务都会去执行refresh定时任务方法

![11_启动两台leadnews-schedule服务](image/Chapter_5/11_启动两台leadnews-schedule服务.png)
...
![12_redis分布式锁](image/Chapter_5/12_redis分布式锁.png)
...
![13_数据库同步到redis](image/Chapter_5/13_数据库同步到redis.png)
...
提供远程的feign接口，在leadnews-feign-api编写类如下：
...
@EnableFeignClients(basePackages = {"com.leadnews.apis","com.leadnews.apis.user"})
@EnableAsync    // 开启异步调用
@EnableScheduling   // 开启调度任务
